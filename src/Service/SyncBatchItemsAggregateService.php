...
namespace BlackCat\Database\Packages\SyncBatchItems\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\SyncBatchItems\Dto\SyncBatchItemDto;
use BlackCat\Database\Packages\SyncBatchItems\Mapper\SyncBatchItemDtoMapper;
 ...
 */
final class SyncBatchItemsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SyncBatchItemRepository $syncBatchItemRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'sync_batch_items' => $this->syncBatchItemRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SyncBatchItems\ModuleException("Record {$idStr} not found for locking.");
            }
