...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'batch_id', 'event_key', 'status', 'error', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'event_key', 'error' ];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        $x = [ 'id', 'batch_id', 'event_key', 'status', 'error', 'created_at' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\SyncBatchItems\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\SyncBatchItems\Definitions::softDeleteColumn();
