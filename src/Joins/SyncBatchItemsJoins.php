...
namespace BlackCat\Database\Packages\SyncBatchItems\Joins;
 ...
 */
final class SyncBatchItemsJoins {
    ...
    /**
     * FK: sync_batch_items -> sync_batches
     * LEFT JOIN vw_sync_batches AS $as ON $as.id = $alias.batch_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinSyncBatches(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_sync_batches AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.batch_id' . ' ', []];
    }
