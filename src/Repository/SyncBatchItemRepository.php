...
namespace BlackCat\Database\Packages\SyncBatchItems\Repository;
...
use BlackCat\Database\Packages\SyncBatchItems\Criteria;
use BlackCat\Database\Packages\SyncBatchItems\Dto\SyncBatchItemDto as Dto;
use BlackCat\Database\Packages\SyncBatchItems\Mapper\SyncBatchItemDtoMapper as RowMapper;
...
final class SyncBatchItemRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SyncBatchItems\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'batch_id', 'event_key' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SyncBatchItems\Definitions::table());
            return $bulk->upsertMany($rows, [ 'batch_id', 'event_key' ], [ 'status', 'error' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SyncBatchItems\Dto\SyncBatchItemDto|null */
    public function getByBatchIdAndEventKey(int $batchId, string $eventKey, bool $asDto = false): array|\BlackCat\Database\Packages\SyncBatchItems\Dto\SyncBatchItemDto|null {
        $row = $this->getByUnique([ 'batch_id' => $batchId, 'event_key' => $eventKey ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SyncBatchItems\Mapper\SyncBatchItemDtoMapper::fromRow($row);
    }
    public function existsByBatchIdAndEventKey(int $batchId, string $eventKey): bool {
        $where = 't.' . Ident::q($this->db, 'batch_id') . ' = :uniq_batch_id' . ' AND ' . 't.' . Ident::q($this->db, 'event_key') . ' = :uniq_event_key';
        return $this->exists($where, [ 'uniq_batch_id' => $batchId, 'uniq_event_key' => $eventKey ]);
    }
    /** @return int|string|null */
    public function getIdByBatchIdAndEventKey(int $batchId, string $eventKey) {
        $row = $this->getByBatchIdAndEventKey($batchId, $eventKey);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SyncBatchItems\Dto\SyncBatchItemDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SyncBatchItems\Dto\SyncBatchItemDto|null {
