...
use BlackCat\Database\Packages\SyncBatchItems\Repository\SyncBatchItemRepositoryInterface;
...
class SyncBatchItemRepository implements SyncBatchItemRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'batch_id', 'event_key' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\SyncBatchItems\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'status', 'error' ]);
          return count($rows);
        }
    ...
    public function getByBatchIdAndEventKey(int $batchId, string $eventKey, bool $asDto = false): array|\BlackCat\Database\Packages\SyncBatchItems\Dto\SyncBatchItemDto|null {
        return $this->getByUnique([ 'batch_id' => $batchId, 'event_key' => $eventKey ], $asDto);
    }
    ...
    public function getIdByBatchIdAndEventKey(int $batchId, string $eventKey) {
        $row = $this->getByBatchIdAndEventKey($batchId, $eventKey, false);
